            ...
            .map(ev => {
                return(
                    <div className="schedule-container" key={ev.id}>
                        <h3>{ev.title}</h3>
                        <p>{moment(ev.start).format('DD MMM')} to {moment(ev.end).format('DD MMM')}</p>
                        <p>From {moment(ev.start).format('HH:mm')} to {moment(ev.end).format('HH:mm')}</p>
                    </div>
                );
            });
        return(
                ...
                <div className="profile-main">
                    <div className="profile-info">
                        <div className="profilepic-container">
                            <img className="profilepic" src={user.pic || "profile.png"}
                    ...
                    <div className="userEvents">
                        <h2>Upcoming for <span className="capitalize">{user.hood}</span></h2>
                        {userEvents}
                    </div>
                </div>
            </div>
